...
def eval_simple(path_hyp, path_refs, n_ref=6):
    ...
    # print('n_ref_desired: %i'%n_ref)
    # print('n_ref_actual: min=%i, max=%i, avg=%.1f'%(min(n_refs), max(n_refs), np.mean(n_refs)))
    temp_dir = os.path.join(os.path.dirname(path_hyp),'temp')
    print(temp_dir)
    ...
    for r in range(n_ref):
        _refs = [refs[i][r%n_refs[i]] for i in range(len(hyps))]
        _path = temp_dir + '/ref%i.txt'%r
        with open(_path, 'w', encoding='utf-8') as f:
            f.write('\n'.join(_refs))
        path_refs.append(_path)
                
    with open(temp_dir + '/hyp.txt', 'w', encoding='utf-8') as f:
        f.write('\n'.join(hyps))

    submitted = temp_dir + '/hyp.txt'
    ...
    print('evaluating %s' % path_hyp)
    print('n_lines = '+str(len(hyps)))
    print('NIST = '+str(nist))
    print('BLEU = '+str(bleu))
    print('METEOR = '+str(meteor))
    print('entropy = '+str(entropy))
    print('diversity = ' + str(distinct))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hyp", type=str, default="/home/yizhe/data/GPT-2/dstc_grounded/")
    parser.add_argument("--ref", type=str)
    args = parser.parse_args()
    path_hyp, path_refs = args.hyp, args.ref
    eval_simple(path_hyp, path_refs)
