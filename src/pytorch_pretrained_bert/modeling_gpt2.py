...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attn_masks=None):
        w = torch.matmul(q, k)
        if self.scale:
        ...
        if attn_masks is None:
            ...
            b = attn_masks.view(attn_masks.shape[0], 1, attn_masks.shape[1], attn_masks.shape[2])[:, :, ns-nd:ns, :ns]#.half()
        #print(b.dtype, w.dtype, self.bias.dtype, q.dtype, k.dtype)
    ...
    def forward(self, x, layer_past=None, attn_masks=None):
        x = self.c_attn(x)
        ...
        a = self._attn(query, key, value, attn_masks=attn_masks)
...
class Block(nn.Module):
    ...
    def forward(self, x, layer_past=None, attn_masks=None):
        a, present = self.attn(self.ln_1(x), layer_past=layer_past, attn_masks=attn_masks)
        x = x + a
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.wtte = nn.Embedding(1+20+10+1, config.n_embd)
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, attn_masks=None, past=None):
        ...
        if token_type_ids is not None:
            ...
            if hasattr(self, 'turn_emb'):
                ...
                token_type_embeds = self.wtte(token_type_ids)
        else:
            token_type_embeds = 0
        hidden_states = inputs_embeds + position_embeds + token_type_embeds
        presents = []
        for block, layer_past in zip(self.h, past):
            hidden_states, present = block(hidden_states, layer_past, attn_masks)
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, lm_labels=None, attn_masks=None, past=None, return_loss=False):
        hidden_states, presents = self.transformer(input_ids, position_ids, token_type_ids, attn_masks, past)
        ...
        if lm_labels is not None:
            ...
            if return_loss:
                token_level_loss = torch.masked_select(loss1, lm_labels.ge(0.0))
            ...
            if return_loss:
                return token_level_loss, loss, ppl
            return loss, ppl
